...
//ex => s="aaaxbbbbyyhwawiwjjjwwm" printer_error(s) => "8/22"

function printerError(s) {
  let x = /[A-M]/gi;
  //regex var, check for letters A thru M
  let n = 0;
  //instances
  let split = s.split("")
  //var to make string into array
  for (let i=0;i<split.length;i++){
    if (split[i].match(x)){
      n++
      //loop and regex match element, if so counter goes up
    }
  } return split.length - n + "/" + split.length
  //substract counter by lenght to return the occurances that did not happen, then length
}
